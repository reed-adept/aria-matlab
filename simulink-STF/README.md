...
This file customizes the Simulink engine and layout. Allowing for files to get registered to a system target file (e.g. system objects used for pil mode) as well as add custom buttons to the Simulink UI. The `sl_customization` for the Aria STF register’s the mex wrapper ‘ext_comm’ to the Aria Target. 
